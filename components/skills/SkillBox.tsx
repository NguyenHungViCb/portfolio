...
const SkillBox: React.FC<{ skill: any }> = ({ skill }) => {
  ...
  return (
    <li className="group flex p-2 relative rounded-lg border border-gray-400 cursor-default">
      ...
      {skill.note && (
        <div className="absolute -top-0 pb-1 -translate-y-full left-1/2 -translate-x-1/2 -z-10 group-hover:z-10">
          <div
            ref={toolTipRef}
            className="breif flex flex-col items-center opacity-0 group-hover:opacity-100  w-max rounded-lg text-white transition-opacity"
          >
